...
namespace PonZ.States
{
  public class GameState : State
  {
    ...
    public override void LoadContent()
    {
      ...
      _enemyManager = new EnemyManager(_content)
      {
        Bullet = bulletPrefab,
            ...
            ///ебанутая ошибка заебали
      _AsteroidManager = new AsteroidManager(_content)
      {
        Bullet = bulletPrefab,
      };
    }
    ...
    public override void Update(GameTime gameTime)
    {
      ...
      /// с какого хуя ты берешь enemy manager а ни asteroid manager
      _AsteroidManager.Update(gameTime);
      if (_AsteroidManager.CanAdd && _sprites.Where(c => c is Asteroid).Count() < _AsteroidManager.MaxAsteroids)
      {
        _sprites.Add(_AsteroidManager.GetAsteroid());
      }
